...
// elenco di nomi
typedef std::set<char *,lstr> charset;
// elenco di nomi associati ad altri nomi
typedef std::map<char *,charset *,lstr> cocmmap; 
...
void addass(char * name1, char * name2)
{
  switch (stat)
	...
	if (i!= shapelist->end())
	  ...
	  fuzzydata->insert(std::pair<char *, charset *> (name1,(*i).second));
	else
	  {
	...
	if (i!= shapelist->end())
	  ...
	  action->insert(std::pair<char *, charset *> (name1,(*i).second));
	else
	  {
	    /* ERRORE */ // non trovata la shape!!!
	    char * msg = (char *) malloc(25 + strlen(name2));
...
void addshape(char * name)
{
  switch (stat)
    ...
    case ctofshape:
      {
	shapelist->insert(std::pair<char *, charset *>(name,tmpcharset));
